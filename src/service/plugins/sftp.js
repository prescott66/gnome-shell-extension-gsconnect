...
var Plugin = GObject.registerClass({
    ...
    async _setup(info) {
        try {
            ...
            // Otherwise just store mountpoint's URI
            } else {
    ...
    async _sftp_mount() {
        try {
            ...
            for (let mount of monitor.get_mounts()) {
                if (this._uri === mount.get_root().get_uri()) {
                    this._mount = mount;
                    break;
                }
            }
    ...
    async _sftp_unmount() {
        try {
            ...
            await new Promise((resolve, reject) => {
                this._mount.unmount_with_operation(1, op, null, (mount, res) => {
                    try {
                        this._mount = null;
    ...
    destroy() {
        // FIXME: unmounting may access plugin variables after finalization
        //        GMounts are often not unmounted suring service shutdown
        this.unmount();
        super.destroy();
    }
});
